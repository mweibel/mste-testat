...
using AutoKlasse = AutoReservation.Common.DataTransferObjects.AutoKlasse;
...
namespace AutoReservation.Service.Wcf
{
	public static class DtoConverter
	{
		...
		{
			if (dto.AutoKlasse == AutoKlasse.Standard)
			{
				return new StandardAuto();
			}
			if (dto.AutoKlasse == AutoKlasse.Mittelklasse)
			{
				return new MittelklasseAuto();
			}
			if (dto.AutoKlasse == AutoKlasse.Luxusklasse)
			{
		...
		private static List<TTarget> ConvertGenericList<TSource, TTarget>(this IEnumerable<TSource> source,
